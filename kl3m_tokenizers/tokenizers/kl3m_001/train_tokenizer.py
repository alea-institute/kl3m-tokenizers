"""
Train a BPE tokenizer from JSONL files.
"""
...
def get_custom_tokens(
    ...
    if include_whitespace:
        # add repeated whitespace like 2-8 consecutive space, \t, \r, \n
        tokens.extend(
            [
                whitespace * multiple
                for whitespace in [" ", "\t", "\r", "\n"]
    ...
    if include_citations:
        ...
        tokens.extend([f"({number})" for number in string.digits])
                ...
                if field_name in record:
                    try:
                        if isinstance(record[field_name], str):
                            ...
                            yield record[field_name].decode("utf-8", errors="replace")
                    except Exception as record_error:  # pylint: disable=broad-except
                        print(f"Unable to parse line from {input_path}: {record_error}")
                        continue
...
def train_tokenizer(
    ...
    print(
        f"Added {len(extend_tokens)} custom tokens to size={tokenizer.get_vocab_size()}."
    )
    ...
    if pad2:
        ...
        for sequence_length in range(1, target_vocab_size):
